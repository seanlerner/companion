class Companion
  class Frame
    ...
    def initialize(args)
      ...
      @data_width = args[:data_width] || Frame.screen_width - title.length
    ...
    def width_from_title_and_data
      height == 1 ? (title.length + data_width) : Frame.screen_width
    end
    ...
    def setup_left(args)
      args[:right] ? Frame.screen_width - width - args[:right] : 0
    end
