class Companion
  class Manager
    def self.update_program_output(filename)
    ...
    def self.execute_monitored_program(filename)
    ...
    def self.line_numer_of_first_modified_line(previous_file, current_file)
    ...
    def self.new_file_diffed(previous_file, current_file)
      Diffy::SplitDiff.new(previous_file, current_file).right
    end

    def self.new_file_diffed_as_separate_lines(file)
      file.split("\n")
    end

    def self.first_diffed_line_of_new_file_diffed(file)
